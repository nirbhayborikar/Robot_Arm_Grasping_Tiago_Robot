...
#    just move and tes
            ...
            if not self.has_rotated:  # Check if function has already run
                self.rotate_robo(-93.0, 1.5)
                self.emergency_brake()
                self.has_rotated = True  # Set flag to True to prevent re-execution
                #self.rotate_robo(-120.0,0.5)
            ##self.rotate_robo(-93.0,1.5)
            ##self.emergency_brake()
            ##self.get_logger().info('rotate worked')
            #self.moving_robot(3.4,1.5)
            #self.get_logger().info('move front worked')

            #self.rotate_robo(90.0,0.5)
            #moving a little forward
            #self.moving_robot(-0.3,-1.0)
            #return journey to previous table
            #self.rotate_robo(220.0,1.0)
            #self.moving_robot(3.3,1.5)
            #self.rotate_robo(-270,1.5)
            #self.moving_robot(-1.2,-1.5)
