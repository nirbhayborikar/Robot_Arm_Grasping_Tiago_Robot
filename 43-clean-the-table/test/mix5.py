'''  
 Object 1 Position: [0.6300816835379237, 0.0790481806162698, 0.5105118461315515] 
 Orientation: [-0.006029272620029044, 0.0003092624851778768, 0.9996201335816123, 0.02689127677646122]
[INFO] [1740675877.844301523] [pick_and_place_object]: 
Object 2 Position: [0.7283313599062533, 0.01625775413262903, 0.5151823547699012]
 Orientation: [-0.0027880079419704278, 0.008023378271953239, 0.9994899913527348, 0.030783268158708594]
[INFO] [1740675878.016974844] [pick_and_place_object]: 

Object 3 Position: [0.6452744655276299, -0.13899684821947408, 0.515109084302803] 
Orientation: [-0.000762423991580803, -0.004836789069635474, 0.9997023493622935, 0.0239005619319671]
...
# adding recheck pose coordinate
        
        self.goal_x = -0.405
        self.goal_y = 5.4614
        self.goal_z = 0.0  # Not used in 2D navigation
        
        self.goal_orientation_z = -0.12
        self.goal_orientation_w = 0.9986
        
        self.position_tolerance = 0.2  # Allowable position error
        self.orientation_tolerance = 0.1  # Allowable orientation error
    ...
    def pose_callback(self, msg):
        """ Callback to check if the robot reached the goal position and orientation """
        current_x = msg.pose.pose.position.x
        current_y = msg.pose.pose.position.y
        current_z = msg.pose.pose.position.z  # Not used in 2D navigation
        ...
        position_reached = (
            abs(current_x - self.goal_x) <= self.position_tolerance and 
            abs(current_y - self.goal_y) <= self.position_tolerance
        )

        # Check if orientation is within tolerance
        orientation_reached = (
            abs(current_orientation_z - self.goal_orientation_z) <= self.orientation_tolerance and 
            abs(current_orientation_w - self.goal_orientation_w) <= self.orientation_tolerance
        )

        # If both position and orientation are reached, start timer
        if position_reached and orientation_reached and not self.reached_goal:
            self.get_logger().info("Goal reached! Starting 2-second timer...")
    ...
    def publish_goal(self):
        ...
        if not self.goal_published:
            ...
            # call
        # Initialize the timer when the goal is reached
