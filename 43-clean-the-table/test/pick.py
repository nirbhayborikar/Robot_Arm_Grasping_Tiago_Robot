...
###### movement object 1 coding 
            ...
            if self.timer_count >= 3:
                ...
                ###### moving and grabbing 2nd object 


                self.__remove_collision_object(object_id=f"collision_obj_{self.marker_id_2}")
                self.get_logger().info('Box Collider removed')

                self.__add_collision_object_1(object_id=f"collision_obj_{self.marker_id_1}",position=[marker_pose_1["pos"][0] + 0.065, marker_pose_1["pos"][1], marker_pose_1["pos"][2]+0.078],orientation=marker_pose_1["rot"],dimensions=[0.04, 0.06, 0.15])
                ...
                # moving to new position
                self.get_logger().info('MOvingto new position')
                self.__move_arm_to_pose(position=new_position_2, orientation=fixed_tagQuat)
                ...
                # Move arm back to a safe position
                self.get_logger().info('Moving arm to safe position')
                safe_position = [lift_position_2[0], lift_position_2[1], lift_position_2[2] + 0.2]
                self.__move_arm_to_pose(position=safe_position, orientation=fixed_tagQuat)

                self.obj_2_lift =True
                time.sleep(5)

                #moving and lifting object 3
                ...
                self.__remove_collision_object(object_id=f"collision_obj_{self.marker_id_3}")
                self.get_logger().info('Box Collider removed')
                self.get_logger().info('adding box 2 ')
                ...
                # moving to new position
                self.get_logger().info('MOvingto new position')
                self.__move_arm_to_pose(position=new_position_3, orientation=fixed_tagQuat)
                ...
                # Move arm back to a safe position
                self.get_logger().info('Moving arm to safe position')
                safe_position = [lift_position_3[0], lift_position_3[1], lift_position_3[2] + 0.2]
                self.__move_arm_to_pose(position=safe_position, orientation=fixed_tagQuat)
